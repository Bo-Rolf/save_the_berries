...
public class View implements ApplicationListener {
    ...
    public void create() {
        ...
        controller = new Controller(model, lawn, viewport);
    ...
    public void render() {
        ...
        // Grid
        float tileW = viewport.getWorldWidth() * 0.75f / lawn.getCols();
        float tileH = viewport.getWorldHeight() * 0.75f / lawn.getRows();
        float gridX = (viewport.getWorldWidth() - tileW * lawn.getCols()) / 2f + 15;
        float gridY = (viewport.getWorldHeight() - tileH * lawn.getRows()) / 2f - 40;
